...
class WirelessInterface:
    picoip = "192.168.0.140"
    porttopico = 9900
    computerip = "192.168.0.158"
    porttocomputer = 9999
...
class WirelessController(CommsController):
    def __init__(self, interface: WirelessInterface):
        ...
        try:
            ...
            self.sockin.bind((self.interface.computerip,
                             self.interface.porttocomputer))
            self.connected = True
        ...
        try:
            while True:
                while self.has_packet():
                    pin = self.get_packet()
                    # Pass incoming packet to function passed into this function
                    func(pin)
        except Exception as e:
